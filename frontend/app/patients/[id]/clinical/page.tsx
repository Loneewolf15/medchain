...
import { getPatient, listClinicalRecords } from "@/lib/api";

export default function ClinicalRecordsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight mb-2">All Clinical Records</h1>
        <p className="text-slate-500">Showing full clinical history for <span className="font-semibold text-slate-700">{patient.full_name}</span> ({patient.patient_code})</p>
      </div>
        ...
        {clinical.map((rec) => (
          <div key={rec.id} className="bg-white/80 backdrop-blur-xl border border-white/60 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all flex flex-col h-full">
            <div className="flex justify-between items-start mb-4">
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800 text-xs uppercase">
                {rec.record_type}
              </span>
              {rec.ledger_tx_id && (
            ...
            <div className="text-xs text-slate-500 mb-3 flex-shrink-0">
              Source: <span className="font-semibold text-slate-700">{rec.source}</span>
            </div>
            
            <div className="flex-1 bg-slate-50/50 rounded-xl p-3 border border-slate-100 overflow-y-auto max-h-[250px]">
              <pre className="text-[10px] sm:text-xs text-slate-700 whitespace-pre-wrap font-mono">
      ...
      {clinical.length === 0 && (
        <div className="text-center py-20 bg-white/50 backdrop-blur-md rounded-3xl border border-white/60 shadow-sm">
          <p className="text-slate-500">No clinical records found for this patient.</p>
        </div>
      )}
    </div>
  );
